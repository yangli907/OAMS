<cfcomponent output="false">
	...
	<cffunction name="read" output="false" access="public" returntype="appmanage.cfcs.mvc.Edit_phone.phoneBean">
		...
		<cfscript>
			obj = createObject("component", "appmanage.cfcs.mvc.Edit_phone.phoneBean").init();
			obj.setid(qRead.id);
	...
	<cffunction name="create" output="false" access="public">
		<cfargument name="bean" required="true" type="appmanage.cfcs.mvc.Edit_phone.phoneBean">
		<cfset var qCreate="">

		<cfset var qGetId="">

		<cfset var local1=arguments.bean.getsid()>
		<cfset var local2=arguments.bean.getphone()>

		<cftransaction isolation="read_committed">
			<cfquery name="qCreate" datasource="AppManage">
				insert into phone(sid, phone)
				values (
			...
			<cfquery name="qGetID" datasource="AppManage">
				...
				where sid = <cfqueryparam value="#local1#" cfsqltype="CF_SQL_INTEGER" null="#iif((local1 eq ""), de("yes"), de("no"))#" />
				  and phone = <cfqueryparam value="#local2#" cfsqltype="CF_SQL_VARCHAR" />
				order by id desc
			</cfquery>
		</cftransaction>
